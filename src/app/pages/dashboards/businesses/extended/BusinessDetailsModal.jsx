    ...
    return (
        <>
            <Transition appear show={open} as={Fragment}>
                <Dialog as="div" className="fixed inset-0 z-[100] overflow-auto" onClose={onClose}>
                    <div className="min-h-screen p-4 text-center">
                        <DialogPanel className="inline-block w-full max-w-7xl p-8 bg-white rounded-lg shadow-xl text-left">
                                ...
                                {directors_names.length > 0 ? (
                                    directors_names.map((d, i) => (
                                        <div key={i} className="mb-4">
                                            <h6 className="text-sm font-semibold mb-1">Director {i + 1}</h6>
                                            <InfoGrid
                                                data={{
                                                    Name: d.name,
                                                    Email: d.email,
                            ...
                            <Section title="Uploaded KYC Documents">
                                {kyc_docs.length > 0 ? (
                                        ...
                                        {kyc_docs.map((doc, idx) => {
                                            ...
                                            return (
                                                <tr key={idx} className="border-b last:border-b-0">
                                                    <td className="py-2 pr-4 text-sm font-medium">{doc.doc_name}</td>
                                                    <td className="py-2 pr-4 text-sm">
                                                      <span className={`text-white text-xs px-2 py-1 rounded-full ${badgeColor}`}>
                                                    ...
                                                    <td className="py-2">
                                                        {(doc.file || doc.url) ? (
                                                            <>
                                                                <Button size="sm" onClick={() => setOpenDocIndex(idx)}>Preview</Button>
                                                                <DocumentReviewModal
                                                            ...
                                                            <span className="text-sm italic text-gray-500">No file</span>
                                                        )}
                                                    </td>
                                                </tr>
                                            );
                                    ...
                                    <p className="text-sm italic text-gray-500">No documents uploaded.</p>
                                )}
                            </Section>
                            ...
                            <Section title="Business Review">
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-end gap-3">
                                        <Button onClick={onClose} variant="flat">Close</Button>
                                        <Button
            ...
            <ConfirmModal
                ...
                messages={{
                    pending: {
                        title: `Confirm Business ${reviewStatus === "verified" ? "Verification" : "Decline"}`,
                        description: "Are you sure?",
                        actionText: "Confirm",
                    },
                    success: {
                        title: `Business ${reviewStatus === "verified" ? "Verified" : "Declined"}`,
