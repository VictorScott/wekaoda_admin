...
export default function DocumentReviewModal({ open, onClose, document, fileUrl, fileType}) {
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            ...
            dispatch(updateDocumentStatusInStore({
        ...
        <>
            <Transition appear show={open} as={Fragment}>
                <Dialog
                    ...
                    onClose={onClose}
                >
                    <DialogPanel className="scrollbar-sm relative max-w-5xl w-full max-h-[90vh] flex flex-col overflow-y-auto rounded-lg bg-white px-6 py-8">
                        <DialogTitle className="text-xl font-semibold">{document.doc_name}</DialogTitle>

                        <div className="mb-6 flex justify-center">
                            {fileType === "application/pdf" ? (
                                <iframe src={fileUrl} title={document.doc_name} className="w-full max-h-[60vh]" />
                            ) : (
                                <img src={fileUrl} alt={document.doc_name} className="max-w-full max-h-[60vh] object-contain" />
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-end space-x-3 pt-4">
                                <Button onClick={onClose} variant="flat" disabled={submitting}>Cancel</Button>
                                <Button
