...
export default function KYCDocTypesDatatable() {

  const { cardSkin } = useThemeContext();

  const dispatch = useDispatch();
  const { kycDocTypes, initialLoading, refreshing, error } = useSelector((state) => state.kycDocTypes);
  ...
  const [toolbarFilters, setToolbarFilters] = useState(["requirement_level", "business_type"]);
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
      "column-visibility-kyc-doc-types",
      {}
  );

  const [columnPinning, setColumnPinning] = useLocalStorage(
      "column-pinning-kyc-doc-types",
      {}
  );
  ...
  useDidUpdate(() => table.resetRowSelection(), [kycDocTypes]);

  useLockScrollbar(tableSettings.enableFullScreen);

  if (initialLoading) {
    return (
        <Page title="KYC Documents">
                            ...
                            <div className="flex flex-col items-center justify-center space-y-3">
                              ...
                              <div className="space-y-1">
                                <p className="text-lg font-medium text-gray-700 dark:text-dark-50">
                                  No KYC document types found
                                </p>
                                <p className="text-sm text-gray-500 dark:text-dark-200">
                                  No KYC document types have been configured yet.
                                </p>
                              </div>
                            </div>
                          </Td>
