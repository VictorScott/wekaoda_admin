...
export function Profile() {
    ...
    // Determine if avatar_color is a valid HEX
    const isHexColor = (color) => /^#([0-9A-F]{3}){1,2}$/i.test(color);

    // If profile_pic is missing and avatar_color is valid, use it
    const avatarColor = !user?.profile_pic && isHexColor(user?.avatar_color)
        ? user.avatar_color
        : undefined;
    ...
    return (
        <Popover className="relative">
            ...
            >
                {!user?.profile_pic && getInitial(user?.name)}
            </PopoverButton>
